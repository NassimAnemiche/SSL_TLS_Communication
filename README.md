...
## Quick Tests
- Round-trip: run client, observe OK/MSG/ERROR replies.
- Max size: send large room message (≈1MB) via jshell to confirm acceptance below limit (10,000,000 bytes).
- Unicode: client sends UTF-8 text; server replies with intact characters.
- Unknown type: client sends type `99`; server replies `ERROR:unknown-type`.
